"""
ovn_intraday_v1 — Overnight → Intraday Fade on MES (single pre-specified variant).

Usage (from repo root):
  python -m backtest.run_ovn_intraday_v1
"""
...
from strategies.overnight_intraday import OvernightIntradayFade

OUT_DIR = Path(__file__).resolve().parents[1] / "analytics" / "output"
STEM = "ovn_intraday_v1"
N_TRIALS = 1  # single pre-specified variant


def load_mes_rth() -> pd.DataFrame:
    """Prefer longest continuous MES series; fall back to 1m 6-month cache."""
...
def _verdict(metrics: dict[str, Any], n_trades: int, worst: float) -> str:
    ...
    # Meets floor but not full success bar (e.g. 1.0 < PF <= 1.2)
    return "FAILED (success criteria not all met)"


def run_ovn_intraday_v1(save: bool = True) -> dict[str, Any]:
    data = load_mes_rth()
    ...
    strategy = OvernightIntradayFade(risk_manager=risk)

    bt = SimpleBacktester(
        strategy=strategy,
        risk_manager=risk,
        symbol="MES",
        stop_points=8.0,  # fallback only; per-trade distance from strategy
        slippage_ticks=1.0,
        bracket_hold=True,
    ...
    # Evaluation table (section 5)
    pf = metrics.get("profit_factor")
    dpf = metrics.get("deflated_profit_factor")
    dsr = metrics.get("deflated_sharpe")
    lines = [
        "=== ovn_intraday_v1 - Overnight -> Intraday Fade (MES) ===",
        ...
        (
            f"Deflated PF:          {dpf:.3f}"
            if dpf is not None
            else "Deflated PF:          n/a"
        ),
        (
            f"Deflated Sharpe:      {dsr:.4f}"
            if dsr is not None
            else "Deflated Sharpe:      n/a"
        ),
